...
use std::vector::{Self, length, empty};
...
#[test]
fun test_add_habits() {
    let mut list: HabitList = empty_list();
    let habit: Habit = make_habit(b"Exercise");
...
fun test_complete_habit() {
    let mut list: HabitList = empty_list();
    let habit: Habit = new_habit(string::utf8(b"Exercise"));
    add_habit(&mut list, habit);
